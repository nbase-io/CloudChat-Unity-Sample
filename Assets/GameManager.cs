...
public class GameManager : MonoBehaviour
{
    ...
    private async Task Connect()
    {
        try
        {
            ...
            if (messages)
            {
                foreach (var message in messages?.Data?.messages?.edges)
                {
                    string id = _nc.ObjectId(message.node.id.ToString());
                    Debug.Log("[ChatSample] id=" + id);
                }
            }
            addMessage("Connected", Message.MessageType.Info);
        }
        catch (InvalidOperationException e)
        {
